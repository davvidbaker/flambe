defmodule FlambeNextWeb.ApiContractTest do
  use FlambeNext.DataCase, async: true

  alias FlambeNext.{Accounts, Traces}
  alias FlambeNextWeb.{ThreadJSON, TraceJSON}

  test "thread show response preserves the legacy JSON contract" do
    user = user_fixture()
    {:ok, trace} = Traces.create_trace(user, %{name: "Contract trace"})
    {:ok, thread} = Traces.create_thread(trace, %{name: "Regression thread", rank: 1})

    assert normalized(ThreadJSON.show(%{thread: thread})) == fixture("thread_show.json")
  end

  test "trace show response preserves the legacy JSON contract" do
    user = user_fixture()
    {:ok, trace} = Traces.create_trace(user, %{name: "Contract trace"})
    ...
    assert normalized(TraceJSON.show(%{trace: trace, events: []})) == fixture("trace_show.json")
  end
  ...
  defp user_fixture do
    {:ok, user} =
      Accounts.create_user(%{
        name: "Contract User",
        username: "contract-user-#{System.unique_integer([:positive])}"
      })
