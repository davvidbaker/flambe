...
class FlameChart extends Component<Props, State> {
      ...
      if (hit) {
        switch (hit.type) {
          ...
          case 'githubLink':
            ...
            this.canvas.style.cursor = 'pointer';
            this.setState({ hoverThreadEllipsis: null, hoverGithubLink: true });
            break;
          /** 💁 hit.value is array like [key, val] */
          case 'block':
            this.props.hoverBlock(hit.value[0]);
        ...
        if (this.state.hoverThreadEllipsis) {
          this.setState({ hoverThreadEllipsis: null, hoverGithubLink: false });
        }
      }
    }
  ...
  render() {
    ...
    const focusedBlock =
      this.props.blocks &&
      this.getBlockDetails(this.canvas && this.props.activities && this.props.focusedBlockIndex);
    const hoveredBlock =
      this.props.blocks &&
      this.getBlockDetails(this.canvas && this.props.activities && this.props.hoveredBlockIndex);

    const hoveredActivity =
      this.props.blocks && hoveredBlock
  ...
  draw() {
    if (this.canvas) {
      ...
      if (this.state.resizing) {
        ...
        // draw vertical bars
        this.drawGrid(this.ctx, this.props.dividersData);
  ...
  drawDraggingThreads() {
    ...
    this.ctx.putImageData(
      this.threadCapture,
      0,
      this.state.cursor.y * window.devicePixelRatio
    );
  }
  ...
  drawBlock(block, activity) {
    ...
    this.ctx.globalAlpha = this.state.hoverGithubLink && sameActivity ? 1 : 0.5;
    if (githubLink) {
      this.ctx.drawImage(
