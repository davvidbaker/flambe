defmodule Flambe.AccountsTest do
  ...
  describe "users" do
    ...
    @valid_attrs %{
      name: "dummy name",
      username: "dummyuser",
      credentials: [%{email: "dummy@email", password: "password"}]
    }
    @invalid_attrs %{
      name: "dummy name",
      username: "dummyuser",
      credentials: [%{email: "dummy@email", password: "p"}]
    }
    ...
    test "with valid data creates inserts user" do
      ...
      assert List.first(user.credentials).email == "dummy@email"
    ...
    test "requires password to be at least 6 chars long" do
      {:error, changeset} = Accounts.register_user(@invalid_attrs)

      assert [%{password: ["should be at least 6 character(s)"]}] =
               errors_on(changeset)[:credentials]
