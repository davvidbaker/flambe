defmodule Stitch.TracesTest do
  ...
  describe "threads" do
    ...
    test "list_threads/0 returns all threads" do
      ...
      assert Traces.list_threads() == [thread]
    end

    test "get_thread!/1 returns the thread with given id" do
      thread = thread_fixture()
      assert Traces.get_thread!(thread.id) == thread
    end

    test "create_thread/1 with valid data creates a thread" do
      assert {:ok, %Thread{} = thread} = Traces.create_thread(@valid_attrs)
      assert thread.name == "some name"
    end

    test "create_thread/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Traces.create_thread(@invalid_attrs)
    end
    ...
    test "update_thread/2 with invalid data returns error changeset" do
      ...
      assert thread == Traces.get_thread!(thread.id)
    end
