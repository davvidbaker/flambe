  ...
  def call(conn, _options) do
    with [agent_id] when byte_size(agent_id) in 1..200 <-
         ...
         {:ok, %{agent: agent, assigned?: assigned?}} <-
           Agents.identify(conn.assigns.current_user, agent_id, provided_name(conn)) do
      conn
      |> assign(:agent, %{id: agent.agent_id, name: agent.name, assigned?: assigned?})
