defmodule Mix.Tasks.Flambe.SeedE2e do
  use Mix.Task
  ...
  def run(_args) do
    unless Mix.env() in [:dev, :test] do
      Mix.raise("flambe.seed_e2e is available only in development and test")
    end
  ...
  defp ensure_activity!(trace) do
    if Traces.list_trace_events(trace) == [] do
      thread =
        Traces.list_trace_threads(trace)
        |> Enum.find(fn item -> item.name == @thread_name end)
        |> case do
          nil ->
